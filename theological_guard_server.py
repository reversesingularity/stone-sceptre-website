...
def validate_scene(scene_text: str, book: str = "?", chapter: str = "?",
    ...
    fast=False: LLM deep-check for every axiom that pattern-matched.
    Returns:
        {
          "passed": bool,
          "violations": [{axiom_id, severity, label, explanation}],
          "warnings": [{axiom_id, severity, label, explanation}],
          "score": 0-100,
          "book": ..., "chapter": ...
        }
    """
    ...
    for axiom in AXIOMS:
        ...
        pattern_hit = any(
            re.search(pat, scene_lower) for pat in axiom.get("violation_patterns", [])
        )

        if pattern_hit or not fast:
            # LLM deep check (skip if fast and no pattern match)
            if fast and not pattern_hit:
                continue

            if fast:
                # Use pattern match as proxy (no LLM)
