// The Nephilim Chronicles Book Five — Coming Soon (Final Volume)
...
function initializeNavigation() {
    ...
    window.addEventListener('scroll', function () {
        if (!activeTicking) {
            requestAnimationFrame(function () { updateActiveNav(); activeTicking = false; });
            activeTicking = true;
        }
    }, { passive: true });
}
...
function updateActiveNav() {
    ...
    document.querySelectorAll('section[id]').forEach(function (sec) {
        if (scrollPos >= sec.offsetTop && scrollPos < sec.offsetTop + sec.offsetHeight) current = sec.id;
    });
...
function initializeScrollEffects() {
    ...
    if (scrollBtn) {
        ...
        window.addEventListener('scroll', function () {
            if (!btnTicking) {
                requestAnimationFrame(function () { scrollBtn.classList.toggle('visible', window.scrollY > 300); btnTicking = false; });
                btnTicking = true;
            }
        }, { passive: true });
    ...
    const observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry, i) {
            if (entry.isIntersecting) {
                setTimeout(function () { entry.target.classList.add('animate-fade-in-up'); }, i * 80);
                observer.unobserve(entry.target);
            }
        });
    }, { threshold: 0.08 });
    document.querySelectorAll('.feature-card,.testimonial-card,.available-card,.section-title,.synopsis-content').forEach(function (el) { observer.observe(el); });
}
