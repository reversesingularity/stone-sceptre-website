...
# PS7+: check all containers in parallel (4x faster than sequential)
$DockerContainers | ForEach-Object -Parallel {
    $container = $_
    $deadline  = (Get-Date).AddSeconds($using:DockerTimeoutSeconds)
    $ready     = $false
    while (-not $ready -and (Get-Date) -lt $deadline) {
        try {
            $status = docker inspect --format '{{.State.Running}}' $container 2>$null
            if ($status -eq 'true') {
                [Console]::WriteLine("[$(Get-Date -Format 'HH:mm:ss')]   [OK] $container is running")
                $ready = $true
            } else { Start-Sleep -Seconds 3 }
        } catch { Start-Sleep -Seconds 3 }
    }
    if (-not $ready) {
        [Console]::WriteLine("[$(Get-Date -Format 'HH:mm:ss')]   [WARN] $container not ready after $($using:DockerTimeoutSeconds)s -- continuing anyway")
    }
} -ThrottleLimit 4
...
foreach ($svc in $Services) {
    ...
    if ($existing) {
        ...
        Log "  [WARN] Port $($svc.Port) in use by PID $procId -- killing orphan"
...
if ($llamaExisting) {
    $procId = $llamaExisting.OwningProcess
    Log "  [WARN] Port $($LlamaServer.Port) in use by PID $procId -- killing orphan"
    Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue
    Start-Sleep -Milliseconds 500
}
...
# --- Phase 6: Health verification (active polling -- no fixed sleep) ---
Log "Phase 6: Health verification (polling until services ready or 60s timeout)..."
$maxWaitSec = 60
$pollInterval = 1
$elapsed = 0
do {
    ...
    foreach ($svc in $Services) {
        $listening = Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue |
            Where-Object { $_.LocalPort -eq $svc.Port }
        if (-not $listening) { $allUp = $false; break }
    }
    if (-not $allUp) { Start-Sleep -Seconds $pollInterval; $elapsed += $pollInterval }
} while (-not $allUp -and $elapsed -lt $maxWaitSec)
if ($elapsed -gt 0) { Log "  Services ready after ${elapsed}s" }
