...
# ── Connection pools (≤3 TCP handshakes per server for the lifetime of the process)
_ollama_session = requests.Session()
_qdrant_session = requests.Session()

# ── Query result cache: (query, category, top_k) → (timestamp, result)
_CACHE_TTL_S: int  = 300  # 5 minutes — canon doesn't change mid-session
_search_cache: dict = {}
...
class CanonSearchHandler(BaseHTTPRequestHandler):
    ...
    def do_search(self, query, category_filter, top_k):
        """Return search results, using the module-level TTL cache for repeat queries."""
        key = (query, category_filter, top_k)
        now = time.time()
        cached_ts, cached_result = _search_cache.get(key, (0, None))
        if cached_result is not None and (now - cached_ts) < _CACHE_TTL_S:
            return {**cached_result, "cached": True}

        try:
            # Get embedding (reuses TCP connection via _ollama_session)
            ...
            # Build Qdrant query (reuses TCP connection via _qdrant_session)
            ...
            _search_cache[key] = (now, result)  # store in cache
            return result
