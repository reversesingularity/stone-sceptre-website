...
# ── Watchdog File Handler (replaces 60s polling) ─────────────────────────────

class NemoclawFileHandler(FileSystemEventHandler):
    """Enqueues modification events to file_event_queue with debounce check."""
    ...
    def on_modified(self, event):
        ...
        age_s = time.time() - mtime
        if age_s < FILE_CHANGE_DEBOUNCE_S:
            # File still being written — the OS will fire another event when flushed
            return
        key = str(fpath)
        if mtime > _file_mtimes.get(key, 0):
            _file_mtimes[key] = mtime
            file_event_queue.put({
                "event_type": "MODIFIED",
                "path":       key,
            ...
            logger.info(f"File change detected (watchdog): {fpath.name}")
...
def main_loop():
    ...
    # Seed _file_mtimes on startup so watchdog only fires on *new* changes
    scan_watched_paths()

    # Start event-driven watchdog observer (replaces FILE_WATCH_INTERVAL polling)
    _handler  = NemoclawFileHandler()
    _observer = Observer()
    for _wp in WATCH_PATHS:
        if _wp.exists():
            _observer.schedule(_handler, str(_wp), recursive=True)
            logger.info(f"Watchdog: monitoring {_wp.name}/")
    _observer.start()
    logger.info("Watchdog observer started — event-driven (was 60s polling).")
