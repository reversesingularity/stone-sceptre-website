...
try:
    ...
    _JERUSALEM_TZ: Optional[object] = ZoneInfo("Asia/Jerusalem")
except Exception:
    _JERUSALEM_TZ = None  # fallback — machine-local time
...
def is_sabbath(now: Optional[datetime] = None) -> bool:
    """Return True if Jerusalem time falls within Friday 18:00 → Saturday 18:00."""
    if now is None:
        now = datetime.now(_JERUSALEM_TZ) if _JERUSALEM_TZ else datetime.now()
...
def time_until_sabbath_ends(now: Optional[datetime] = None) -> timedelta:
    """Return how long until the Sabbath ends (Saturday at sundown, Jerusalem time)."""
    if now is None:
        now = datetime.now(_JERUSALEM_TZ) if _JERUSALEM_TZ else datetime.now()
