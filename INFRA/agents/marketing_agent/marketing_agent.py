...
# Ensure parent agents/ directory is on path so governance.py is importable
_AGENTS_DIR = _PKG_DIR.parent
...
def _log_generation(req: ContentRequest, resp: ContentResponse) -> None:
    """Append generation record to marketing_log.json and shared agent_audit.jsonl."""
    entry = {
    ...
    # Write to shared governance audit log so PIMMUR and conductor can audit agent 13
    try:
        from governance import log_invocation  # lazy import — avoids circular dep risk
        log_invocation(
            agent_id="AGENT_13",
            tool_name="content_generation",
            args={"content_type": req.content_type, "book": req.book},
            result={"preview": resp.variants.short[:120]},
            status="success",
        )
    except Exception:
        pass  # governance logging is non-critical; never block content generation
